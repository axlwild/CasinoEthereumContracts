...
contract TragamonedasSP /*is usingProvable*/ {
    ...
    function calcularPremio() private view returns (uint){
        ...
        if ((casillasObtenidas[0] == casillasObtenidas[1]) && (casillasObtenidas[1] == casillasObtenidas[2])){
            if      (casillasObtenidas[0] == 1) return PREMIO_BASE;     //Gana 0.25 ETH.
            else if (casillasObtenidas[0] == 2) return PREMIO_BASE * 2; //Gana 0.5 ETH.
            else if (casillasObtenidas[0] == 3) return PREMIO_BASE * 4; //Gana 1.0 ETH.
            else if (casillasObtenidas[0] == 4) return PREMIO_BASE * 8; //Gana 2.0 ETH.
            else if (casillasObtenidas[0] == 5) return PREMIO_BASE * 20;//Gana 5.0 ETH.
        }
    ...
    function verEstado() public view returns (string memory estado) {
        if (!jugando) return "El juego no está activo. Presiona jugar() para 'jalar la palanca' de la máquina.";
        else if (jugando && casillasObtenidas.length < 3) return "El juego ya está activo, pero todavía se están generando los resultados. No podrá empezar otro juego hasta que se consulten los resultados de este.";
        else if (jugando && casillasObtenidas.length >= 3) return "Ya se pueden consultar los resultados. Una vez consultados, podrá empezar un nuevo juego.";
    ...
    function generar3Aleatorios() private {
        ...
        for (uint8 i = 0; i < 3; i++) {
            ...
            casillasObtenidas.push(random(i));
        }
    }
    
    function random(uint seed) private view returns (uint) {
       return (uint256(keccak256(abi.encodePacked(block.timestamp, block.difficulty, seed)))%5)+1;
    }
